...
async function updateContact({ contactId, locationId, customFieldId }) {
    console.log(`🔔 Actualizando contacto ${contactId}`);
    const res = await fetch(`https://gh-connector.vercel.app/proxy/contacts/${contactId}`, {
        method: 'PUT',
        headers: {
...
new Worker('contactos', async (job) => {
